...
let MaintenanceService = class MaintenanceService {
    ...
    async createRequest(propertyId, nriId, data) {
    ...
    async updateStatus(requestId, status, rmId, userRoles) {
        if (!userRoles.includes('RELATIONSHIP_MANAGER') && !userRoles.includes('ADMIN')) {
            throw new common_1.ForbiddenException('Only RM or Admin can update status');
        }
